let mnist_cnn, reader = new deeplearn.CheckpointLoader('./alphabet_ckpt.9826');
...
reader.getAllVariables().then(function (vars) {
  ...
  document.querySelector("th").innerHTML = "Please write an alphabet and press infer!";
});
...
function buildModelMathAPI(math, vars) {
  ...
  return function (x) {
    return math.scope(function (keepFn, trackFn) {
      const x_image = deeplearn.Array3D.new([40, 40, 1], x);
      ...
      const h_pool2_flat = h_pool2.reshape([1600]);
